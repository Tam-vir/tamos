...
{
    interrupt_register(UART_IRQ, uart_interrupt_handler);
...
{
    while (!(uart[UART_LSR] & UART_LSR_THR_EMPTY))
    {
        asm volatile("wfi");
    }
    uart[UART_THR] = c;
...
void uart_interrupt_handler(void)
{
    ...
    {
        uint8_t c = uart[UART_RBR];
        ring_push(&uart_rx_buffer, c);
    }
    spinlock_unlock(&uart_lock);
    waitqueue_wake_all(&uart_waitqueue);
}
